...
namespace KsmUI
{
	public class KsmUIContextMenu : MonoBehaviour
	{
		private const float hoverOffset = 25f;

		public static KsmUIContextMenu Instance { get; private set; }

		private RectTransform topTransform;
		private KsmUIVerticalLayout content;

		private GameObject menuObject;
		private ContentSizeFitter topFitter;

		private KsmUIBase target;
		...
		private void Awake()
		{
			...
			menuObject = new GameObject("KsmUIContextMenu");
			...
			topTransform.sizeDelta = new Vector2(KsmUIStyle.tooltipMaxWidth, 0f); // max width of tooltip, text wrap will occur if larger.
			...
			menuObject.AddComponent<KsmUIContextMenuArea>();

			//GameObject mouseOver = new GameObject("KsmUIMenuMouseOver");
			...
			//mouseOver.AddComponent<KsmUIContextMenuArea>();

			// first child : 1px white border
			GameObject border = new GameObject("KsmUIMenuBorder");
			...
			GameObject background = new GameObject("KsmUIMenuBackground");
			...
			// last child : content
			content = new KsmUIVerticalLayout(null);
		...
		public KsmUIVerticalLayout Create(KsmUIBase target, TextAnchor anchor = TextAnchor.UpperLeft)
		{
			this.target = target;
			this.anchor = anchor;
		...
		public KsmUITextButton AddButton(string label, Action callback)
		{
			...
			return new KsmUITextButton(content, label, CloseCallback);
		}
		...
		private class KsmUIContextMenuArea : MonoBehaviour, IPointerExitHandler
		{
			public void OnPointerExit(PointerEventData pointerEventData)
			{
				KsmUIContextMenu.Instance.Destroy();
			}
		}
	}
}
